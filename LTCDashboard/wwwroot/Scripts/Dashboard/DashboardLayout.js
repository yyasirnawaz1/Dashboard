...
var Dashboard = {
    ...
    filterTypeHTML:'<div class="form-check"><label class="form-check-label"><div class="uniform-checker border-primary text-primary"><span class="checked" data-name="{name}"><input id="chkFilterType-{name}" type="checkbox" class="form-check-input-styled-primary" checked="" value="{name}" data-fouc=""></span></div>{name}</label></div>',
    ...
    openFilterTypesModel: function (id, selectedTypes) {
        Dasbhaord.selectedchartname = id;
        ...
        for (let i = 0; i < selectedTypes.split(',').length; i++) {
    ...
    saveFilterTypeOptions: function () {
        ...
        const selectFilterTypes = $("#allowedFilterTypes span.checked");
        let filterTypes = '';
        ...
        $.ajax({
            ...
            data: {
                chartId: Dasbhaord.selectedchartname,
